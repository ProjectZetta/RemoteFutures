...
/**
 * Created by mse on 21.05.14.
 */
class ExecutionContextImpl (id: Int, es: Executor, reporter: Throwable => Unit) extends ExecutionContextExecutor {
  ...
  def execute(runnable: Runnable): Unit = {
    println("Running execution context " + id + ".")
    executor match {
...
object ExecutionContextImpl {
  def fromExecutor(id: Int, e: Executor, reporter: Throwable => Unit = ExecutionContext.defaultReporter): ExecutionContextImpl = new ExecutionContextImpl(id, e, reporter)
  def fromExecutorService(id:Int, es: ExecutorService, reporter: Throwable => Unit = ExecutionContext.defaultReporter): ExecutionContextImpl with ExecutionContextExecutorService =
    new ExecutionContextImpl(id, es, reporter) with ExecutionContextExecutorService {
