      ...
      case fun @ Function(vparams, body) =>
        ...
        def isSymbolValid(s: Symbol): Boolean =
          validEnv.contains(s) ||
            ...
            s.owner == fun.symbol ||
            s.isStatic || {
            c.error(s.pos, "[Spores]: (symbol) invalid reference to " + s)
            false
          }
      ...
      case fun @ Function(vparams, body) =>
        ...
        def isSymbolValid(s: Symbol): Boolean =
          validEnv.map(_._1).contains(s) ||
            s.owner == fun.symbol ||
            s.isStatic || {
            c.error(s.pos, "[Spores]: (function) invalid reference to " + s)
            false
          }
        ...
        val traverser = new Traverser {
          override def traverse(tree: Tree) {
            tree match {
              ...
              case sel @ Select(app: Apply, _) =>
                debug("checking select (app) (function) " + sel)
                if (app.symbol.isStatic) {
                  debug("OK, fun static")
                } else c.error(sel.pos, "[Spores] (function): the fun is not static")
